...
def model1(g_temp,g_cond,idmat,flow_init,queue_init,g_nc_min,g_nc_max,g_c_min,g_c_max,K,cont=0,g_form=0 ):
    ...
    for i in range(20):
        ...
        for j in range(15):
            for k in range(2):
                g_sum+=g[j][i*2+k]
        if g_sum>110*60:
            g_sum_con+=100*(g_sum-(23*60))**2
    #print g
    fitness=-(D_temp-q_temp-fitness_q-fitness_beta-fitness_g-g_sum_con)
    #print fitness
    return(fitness)
