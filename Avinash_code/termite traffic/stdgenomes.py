...
class FloatGenome(BaseGenome):
    ...
    def __init__(self, initial, init_limits=(0,1),
        ...
        if not spawn_chances:
            spawn_chances = ((FloatGenome.fresh, 1),
                             (FloatGenome.copy, 1),
                             (FloatGenome.crossover, 2),
                             (FloatGenome.big_mutate, 5),
                             (FloatGenome.medium_mutate, 2),
                             (FloatGenome.small_mutate, 1))
    ...
    def crossover(self):
        ...
        #print('child_genes'),child.genes
        for offspring in xrange(len(child.genes)):
            if child.genes[offspring]>self.init_limits[1]:
                child.genes[offspring] = child.genes[offspring]-self.init_limits[1]+self.init_limits[0]
            elif child.genes[offspring]<self.init_limits[0]:
                child.genes[offspring] = child.genes[offspring]-self.init_limits[0]+self.init_limits[1]
        return child
...
class PermutateGenome(BaseGenome):
    ...
    def swap(self):
        ...
        """
        child = self.copy()
        if bestfit<25 and bestfit>10:
            for k in range(9):
                i = random.randrange(len(child.genes))
                j = random.randrange(len(child.genes))
                child.genes[i], child.genes[j] = child.genes[j], child.genes[i]
        elif bestfit<=10 and bestfit>5:
            for k in range(18):
                i = random.randrange(len(child.genes))
                j = random.randrange(len(child.genes))
                child.genes[i], child.genes[j] = child.genes[j], child.genes[i]
